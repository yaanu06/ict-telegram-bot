...
async function getQuote(tfStr){
    ...
    try{ const r=await fetch(`${TWELVE_DATA_BASE}/quote?symbol=${encodeURIComponent(SYMBOLS[pair])}&interval=${interval}&apikey=${TWELVE_DATA_KEY}`); const d=await r.json(); if(d.open && d.close){calls++;return{open:+d.open,close:+d.close,is_market_open:d.is_market_open};} }catch(e){}
    return null;
}

// 🟢 LIVE CANDLE DIRECTION (FOR UI DASHBOARD ONLY)
async function getLiveCandleDirection(tfStr) {
    try {
...
async function getHistory(tfStr){
    if(!TWELVE_DATA_KEY)return null;
    try{ const r=await fetch(`${TWELVE_DATA_BASE}/time_series?symbol=${encodeURIComponent(SYMBOLS[pair])}&interval=${TF_MAP[tfStr]}&outputsize=100&apikey=${TWELVE_DATA_KEY}`); const d=await r.json(); if(d.values){calls++;return d.values.map(c=>({t:c.datetime,o:+c.open,h:+c.high,l:+c.low,c:+c.close,v:+c.volume||1e6})).reverse();} }catch(e){}
    return null;
}
...
async function updateMTFDisplay(){
    ...
    for(let t of tfs){
        let tr = await getLiveCandleDirection(t);
...
function getSession() { const hour = new Date().getUTCHours(); if (hour >= 8 && hour <= 11) return { session: 'LONDON', multiplier: 1.0, emoji: '🇬🇧' }; if (hour >= 13 && hour <= 16) return { session: 'NEW_YORK', multiplier: 1.0, emoji: '🇺🇸' }; if (hour >= 0 && hour <= 4) return { session: 'ASIA', multiplier: 0.85, emoji: '🌏' }; return { session: 'OFF-HOURS', multiplier: 0.6, emoji: '🌙' }; }
...
async function analyzeTimeframe(tfToAnalyze, price, htfData) {
    try {
        ...
        const structureData = await getHistory(structureTF), twelveIndicators = await getTechnicalIndicators(tfToAnalyze), sig = score(entryData, price, twelveIndicators);
        ...
        const magnetism = checkZoneMagnetism(entryData, price, entry, direction), displacement = detectDisplacement(entryData, direction), sniperRej = await checkSniperRejection(zone, direction, sniperTF);
        const probCheck = checkProbability(zone, mtf, magnetism), slResult = calcStopLoss(entryData, direction, entry, zone, msnr, tfToAnalyze, twelveIndicators), tps = calcTakeProfits(direction, entry, slResult.price);
        ...
        conf = Math.min(conf * session.multiplier, 98); if (breakerValid) conf = Math.min(conf + 8, 98);
...
function calculateSetupQuality(result, price) {
    ...
    if (result.session?.multiplier >= 0.85) score += 10; else score -= 10; if (result.breakerValid) score += 8;
...
async function askAIWithAllResults(allResults, price, htfData) {
    ...
    const best = allResults[0], prec = getPrec(pair), dailyDir = await getQuoteDirection('1D'), h4Dir = await getQuoteDirection('4H'), htfConfluence = await checkHTFConfluenceAsync(htfData['1D'], htfData['4H'], best.direction);
...
async function runAutoScan() {
    ...
    try {
        ...
        const mtfTrendsData = {}; const tfs = ['5M', '15M', '1H', '4H', '1D']; for (let t of tfs) mtfTrendsData[t] = await getQuoteDirection(t);
        await updateMTFDisplay(); document.getElementById('currentPrice').innerHTML = `$${price.toFixed(getPrec(pair))}`;
        ...
        const results = [], timeframesToScan = ['1D', '4H', '1H', '15M', '5M'], htfData = {};
        scanText.innerHTML = 'Loading HTF context...'; const dailyData = await getHistory('1D'), h4Data = await getHistory('4H');
