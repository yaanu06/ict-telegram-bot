...
async function analyzeTimeframe(tfToAnalyze, price, htfData) { 
    try { 
        ...
        const tfs = ['5M', '15M', '1H', '4H', '1D']; let bullCount = 0, bearCount = 0; const trends = {}; 
        ...
        const turtleSoup = detectTurtleSoup(entryData); // Keep TBS for validation but don't override HTF direction 
        ...
        const amd = analyzeAMD(htfData['1H'] || htfData['1D']); 
        const breakersFound = detectBreakers(entryData); let breakerValid = false; 
        if (breakersFound.length > 0) { const relevantBreaker = breakersFound.find(b => direction === 'BUY' ? b.type === 'BULL' : b.type === 'BEAR'); if (relevantBreaker) breakerValid = validateBreakerBlock(entryData, relevantBreaker.p, direction); } 
        const session = getSession(); let htfValidation = { passed: true, parentArray: null, structureTF: structureTF }; 
        if (structureTF !== entryTF && structureData && structureData.length >= 20) { const structureArrays = findPDArrays(structureData, direction), validation = isZoneWithinHTFArray(zone, structureArrays); if (!validation.contained) htfValidation = { passed: false, parentArray: null, structureTF: structureTF }; else htfValidation = { passed: true, parentArray: validation.parentArray, structureTF: structureTF }; } 
        let entry = null, entryReady = false; 
        if (zoneReaction.confirmed && (zoneReaction.strength === 'STRONG' || zoneReaction.strength === 'MODERATE')) { entryReady = true; entry = calculatePrecisionEntry(entryData, zone, direction); } 
        ...
        if (zoneReaction.confirmed && (zoneReaction.strength === 'STRONG' || zoneReaction.strength === 'MODERATE')) conf = Math.min(conf + 20, 98); else if (zoneTouches === 0 && zone.quality === 'A') conf = Math.min(conf + 25, 98); else if (!zoneReaction.confirmed) conf = Math.max(conf - 5, 10);
        if (zoneTouches >= 5 && !zoneReaction.confirmed) conf = Math.max(conf - 5, 10); if (zoneTouches >= 2 && zoneReaction.confirmed) conf = Math.min(conf + 10, 98);
...
function calculateSetupQuality(result, price) { 
    ...
    if (result.entryReady) score += 20; if (riskPct < 0.1) score -= 10; if (riskPct > 2.0) score -= 10;
...
async function runAutoScan() { 
    ...
    try { 
        ...
        const setupTimeframes = ['4H', '1H'], lowerTimeframes = ['15M', '5M']; 
        const setupResults = results.filter(r => setupTimeframes.includes(r.timeframe)), lowerResults = results.filter(r => lowerTimeframes.includes(r.timeframe)); 
        let best = null, isLowerTF = false; 
        if (setupResults.length > 0) { setupResults.sort((a, b) => b.qualityScore - a.qualityScore); best = setupResults[0]; isLowerTF = false; showNotif(`✅ ${best.timeframe} setup found - Quality: ${best.qualityScore}%`, 'success'); }  
        ...
        if (ghostChecks.length > 0) {
            best.confidence = Math.min(best.confidence, 30); // Heavily penalize to fail the 70% threshold
            aiKeyReason = "Ghost Machine Rules Failed: " + ghostChecks.join(', ');
        ...
        const out = { trade_signal: { date: new Date().toISOString().split('T')[0], time: new Date().toISOString().split('T')[1].split('.')[0], pair, current_price: price, trade_type: best.direction === 'BUY' ? 'BUY' : 'SELL', entry_price: finalEntry, stop_loss: best.sl, take_profit_1: best.tp1, take_profit_2: best.tp2, take_profit_3: best.tp3, confidence: best.confidence, setup_score: best.qualityScore, zone_type: best.zone.src, zone_price: ((best.zone.low + best.zone.high) / 2).toFixed(getPrec(pair)), patterns_detected: best.zone.confluence, zone_touches: best.zoneTouches, zone_freshness: best.freshness.fresh ? 'FRESH' : (best.freshness.partiallyUsed ? 'PARTIALLY_USED' : 'USED'), entry_distance_pct: ((Math.abs(price - finalEntry) / price) * 100).toFixed(2) + '%', htf_alignment: best.mtf.strength + '/5', entry_atr: best.apiATR.toFixed(getPrec(pair)), adx_trend_strength: (best.twelveIndicators.adx || 0).toFixed(1) + (best.twelveIndicators.adx > 25 ? ' (Strong > 25)' : ' (Weak < 25)'), dynamic_risk_percent: ((risk / best.entry) * 100).toFixed(2) + '%', trade_management_rules: { recommended_risk_pct: ((risk / best.entry) * 100).toFixed(2) + '%', sl_to_breakeven: 'Move Stop Loss to Entry price after TP1 is hit', partial_take_profits: { tp1: 'Close 50% position at TP1', tp2: 'Close 30% position at TP2', tp3: 'Close remaining 20% position at TP3' }, trailing_stop_loss: 'Trail SL behind 15M market structure / ATR buffer after TP1' }, tbs_detected: best.turtleSoup?.detected ? 'YES' : 'NO', crt_state: best.crt.detected ? (best.crt.pattern || 'DETECTED') : 'NONE', ai_decision: { decision: executionDecision === 'enter_now' ? 'enter_now' : (executionDecision === 'wait_for_reaction' ? 'wait_confirmation' : 'skip'), confidence: best.confidence, reasoning: aiKeyReason || `${best.zone.confluence} [Q:${best.zone.quality}]`, risk_adjustment: aiConfAdj, wait_condition: executionDecision !== 'enter_now' ? waitCondition : null, skip_reason: executionDecision === 'skip' ? ghostChecks.join(', ') : null }, analysis: { timeframe: best.timeframe, session: session.session, silver_bullet: session.isSilverBullet ? '✅' : '❌', killzone: session.isKillzone ? '✅' : '❌', risk_reward: '1:' + rrDisplay }, msnr_levels: { pivot: best.msnr.pivot.toFixed(getPrec(pair)), support_1: best.msnr.supports.S1?.toFixed(getPrec(pair)), support_2: best.msnr.supports.S2?.toFixed(getPrec(pair)), support_3: best.msnr.supports.S3?.toFixed(getPrec(pair)), resistance_1: best.msnr.resistances.R1?.toFixed(getPrec(pair)), resistance_2: best.msnr.resistances.R2?.toFixed(getPrec(pair)), resistance_3: best.msnr.resistances.R3?.toFixed(getPrec(pair)), nearest_support: best.msnr.supports.S1?.toFixed(getPrec(pair)), nearest_resistance: best.msnr.resistances.R1?.toFixed(getPrec(pair)) }, trade_levels: { entry: finalEntry.toFixed(getPrec(pair)), stop_loss: best.sl.toFixed(getPrec(pair)), take_profit_1: best.tp1.toFixed(getPrec(pair)), take_profit_2: best.tp2.toFixed(getPrec(pair)), take_profit_3: best.tp3.toFixed(getPrec(pair)), risk_reward_ratio: parseFloat(rrDisplay), use_partial_profits: rr > 2 ? 'YES (RR > 2x)' : 'NO' } } };
