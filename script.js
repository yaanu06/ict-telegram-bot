...
function init(){
    ...
    document.getElementById('analyzeBtn').addEventListener('click',runAutoScan);
...
function findPrecisionEntry(data,price,direction,msnr){
    ...
    if(direction==='BUY'){const low=rL+r*.618,high=rL+r*.79;return{low,high,p:(low+high)/2,src:'OTE',confluence:'OTE',cc:1,quality:'C',hasImbalance:false};}
    else {const low=rH-r*.79,high=rH-r*.618;return{low,high,p:(low+high)/2,src:'OTE',confluence:'OTE',cc:1,quality:'C',hasImbalance:false};}
}
...
function calcStopLoss(data,dir,entry,zone,msnr,tfUsed,twelveIndicators){
    ...
    const addCandidate = (price, reason, priority) => {
        ...
        if (d > 0 && d <= maxSLD * 1.5) c.push({ price, reason, distance: d, priority });
    };
    if(dir === 'BUY'){
    ...
    c.sort((a, b) => { if (b.priority !== a.priority) return b.priority - a.priority; return dir === 'BUY' ? a.price - b.price : b.price - a.price; });
    if (c.length > 0) { const best = c[0]; return { price: best.price, reason: best.reason, distance: best.distance }; }
...
function validateBreakerBlock(data, level, direction) {
    ...
    const touched = recent.some(c => direction === 'BUY' ? c.l <= level : c.h >= level);
...
async function analyzeTimeframe(tfToAnalyze, price, htfData) {
    try {
        ...
        if (breakersFound.length > 0) {
            const relevantBreaker = breakersFound.find(b => direction === 'BUY' ? b.type === 'BULL' : b.type === 'BEAR');
            if (relevantBreaker) breakerValid = validateBreakerBlock(entryData, relevantBreaker.p, direction);
        }
        ...
        if (structureTF !== entryTF && structureData && structureData.length >= 20) {
            ...
            if (!validation.contained) htfValidation = { passed: false, parentArray: null, structureTF: structureTF };
            else htfValidation = { passed: true, parentArray: validation.parentArray, structureTF: structureTF };
        }
        ...
        if (htfValidation.passed) conf = Math.min(conf + 15, 98); 
        else if (structureTF !== entryTF) conf = Math.max(conf - 20, 10);
        if (zoneReaction.confirmed && (zoneReaction.strength === 'STRONG' || zoneReaction.strength === 'MODERATE')) conf = Math.min(conf + 20, 98);
        else if (!zoneReaction.confirmed) conf = Math.max(conf - 20, 10);
        ...
        return {
            ...
            htfValidation, freshness, premiumDiscount, session, breakerValid, hasSweep
        };
...
function calculateSetupQuality(result, price) {
    ...
    if (result.zone.quality === 'A') score += 20; else if (result.zone.quality === 'B') score += 10;
    ...
    if (riskPct < 0.1) score -= 10; if (riskPct > 2.0) score -= 10;
    ...
    if (result.session?.multiplier >= 0.85) score += 10; else score -= 10;
...
async function runAutoScan() {
    ...
    try {
        ...
        if (higherResults.length > 0) {
            ...
            best = higherResults[0]; isLowerTF = false;
            ...
            if (filteredLower.length > 0) {
                ...
                best = filteredLower[0]; isLowerTF = true;
                ...
                document.getElementById('jsonOutput').innerHTML = JSON.stringify({auto_scan_result:{date:new Date().toISOString().split('T')[0],time:new Date().toISOString().split('T')[1].split('.')[0],pair,current_price:price,status:'LOW_QUALITY_SETUPS_ONLY',message:'Only low quality lower timeframe setups found. Not tradable.',multi_timeframe_trends:mtfTrendsData,lower_setups_found:lowerResults.length,best_quality:Math.max(...lowerResults.map(r=>r.qualityScore))}}, null, 2);
                analysis = null; document.getElementById('executeBtn').disabled = true; btn.classList.remove('loading'); btn.disabled = false; scanStatus.classList.add('hidden'); return;
            }
        } else {
            showNotif('⚠️ No valid setups found', 'warning');
            document.getElementById('jsonOutput').innerHTML = JSON.stringify({auto_scan_result:{date:new Date().toISOString().split('T')[0],time:new Date().toISOString().split('T')[1].split('.')[0],pair,current_price:price,status:'NO_SETUP',multi_timeframe_trends:mtfTrendsData,timeframes_scanned:timeframesToScan.length}}, null, 2);
        ...
        const out = {
            auto_scan_result: {
                date: new Date().toISOString().split('T')[0], time: new Date().toISOString().split('T')[1].split('.')[0],
                pair, current_price: price, multi_timeframe_trends: mtfTrendsData, best_timeframe: best.timeframe,
                quality_score: best.qualityScore, total_setups_found: results.length, higher_timeframe_setups_found: higherResults.length,
                lower_timeframe_setups_available: lowerResults.length, is_lower_timeframe_signal: isLowerTF,
                signal_quality: isLowerTF ? 'LOWER_TF_ONLY_REDUCED_CONFIDENCE' : 'HIGHER_TF_TRADABLE',
                session: session.session, session_emoji: session.emoji, session_multiplier: session.multiplier,
                premium_discount: best.premiumDiscount, zone_freshness: best.freshness, breaker_validated: best.breakerValid,
                ai_verified: !!aiResult, ai_approved: aiApproved, execution_decision: executionDecision, wait_condition: waitCondition || null,
                htf_confluence: htfConfluence,
                trade_signal: {
                    trade_type: best.direction === 'BUY' ? 'BUY-LIMIT' : 'SELL-LIMIT', entry_price: finalEntry,
                    entry_zone: { low: finalZoneLow, high: finalZoneHigh }, entry_ready: best.entryReady, zone_touches: best.zoneTouches,
                    ...
                    stop_loss: best.sl, sl_reason: best.slResult.reason, invalidation_price: aiInvalidation,
                    risk_amount: risk.toFixed(prec), stop_loss_pct: ((risk / best.entry) * 100).toFixed(2) + '%',
                    take_profit_1: best.tp1, take_profit_2: best.tp2, take_profit_3: best.tp3,
                    risk_reward: '1:' + rrDisplay, dynamic_rr: '1:' + rr, confidence: best.confidence, conviction: aiConviction,
                    entry_source: aiResult ? 'AI-Refined' : 'Rule-Based', ai_used: !!aiResult, ai_risk_warning: aiRiskWarning || null,
                    entry_reasoning: aiEntryLogic || `${best.zone.src} zone with ${best.zone.confluence}`, sl_reasoning: aiSlLogic || best.slResult.reason,
                    ...
                    zone_quality: best.zone.quality, zone_source: best.zone.src, zone_confluence: best.zone.confluence, confluence_count: best.zone.cc,
                    imbalance_magnet: best.zone.hasImbalance, zone_reaction: best.zoneReaction,
                    zone_magnetism: { strength: best.magnetism.magnetism, score: best.magnetism.score, summary: best.magnetism.summary, checks: best.magnetism.checks },
                    path_clearance: { clear: best.pathCheck.clear, obstacles: best.pathCheck.obstacles }, probability: best.probCheck.probability,
                    timeframe_alignment: {
                        trend_tf: best.trendTF, structure_tf: best.structureTF, entry_tf: best.entryTF, sniper_tf: best.sniperTF,
                        alignment: best.tfAlign, trend_direction: best.mtf.direction, trend_strength: best.mtf.strength + '/5 TFs',
                        sniper_confirmation: best.sniperRej.confirmed ? '✅ Confirmed' : '⚠️ No rejection', htf_confluence: htfConfluence
                    },
                    turtle_soup: best.turtleSoup, crt_analysis: best.crt, order_blocks_found: best.obsAll ? best.obsAll.length : 0, twelve_data_indicators: best.twelveIndicators,
                    msnr_levels: {
