...
// ============================================
// 🛡️ FIXED: ATR-BASED STOP LOSS (Uses Twelve Data ATR)
// ============================================
function calcStopLoss(data, dir, entry, zone, msnr, tfUsed, twelveIndicators, currentPair) {
    ...
    // Get ATR-based buffer (Ghost Machine uses 1.0-1.2x ATR for SL)
    const atrMultiplier = 1.2;
    const slBuffer = Math.max(apiATR * atrMultiplier, s.slBuffer);
    
    // CLAMP: SL must be reasonable (not too tight, not too wide)
    const minSL = s.minSL;
    ...
    if (dir === 'BUY') {
        ...
        // CRT extreme (Ghost Machine style)
        const crtRange = {
...
async function analyzeTimeframe(tfToAnalyze, price, htfData) {
    ...
    try {
        ...
        // Log Twelve Data ATR for debugging
        ...
        for (const dir of ['BUY', 'SELL']) {
            ...
            // ============================================
            // FIXED: ENTRY AT ZONE EDGE USING ATR
            // ============================================
            let entry;
            if (dir === 'BUY') {
                // Entry at zone LOW + 0.15 ATR (Ghost Machine style)
                entry = Math.min(zone.low + entryATR * 0.15, zone.high);
            } else {
                // Entry at zone HIGH - 0.15 ATR (Ghost Machine style)
                entry = Math.max(zone.high - entryATR * 0.15, zone.low);
            }

            // ENTRY DISTANCE CHECK: Must be within 1.5 ATR of price
            ...
            // ============================================
            // FIXED: SL USING TWELVE DATA ATR
            // ============================================
