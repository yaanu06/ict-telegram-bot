...
async function analyzeTimeframe(tfToAnalyze, price, htfData) {
    ...
    try {
        ...
        if (volumeProfile) {
            ...
            if (zoneInLVN) conf += 5;
            if (nearVAEdge) conf += 5;
        }
        if (deltaProxy.direction === (sig.dir === 'BUY' ? 'BULLISH' : 'BEARISH')) conf += 4;
        else if (deltaProxy.direction !== 'NEUTRAL') conf -= 4;
        ...
        if (!pathCheck.clear) conf -= 8;
        if (probCheck.probability === 'LOW') conf -= 8;
