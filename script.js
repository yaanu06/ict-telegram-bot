...
function score(data, price) {
    ...
    const fv = fvg(data), ms = mss(data), bk = breakers(data);
    ...
    if(bS>sS&&bS>=45){ dir='BUY'; conf=Math.min(bS+10,95); reason=bR.join('; ');
        ...
        else if(sB.length) zone={p:sB[0].p,l:sB[0].p-a*.3,h:sB[0].p+a*.3,src:'Breaker'};
    ...
    if(zone){ const buf=pair.includes('XAU')?2:a*.2; sl=dir==='BUY'?zone.l-buf:zone.h+buf; }
    return {dir,conf,zone,sl,reason,scores:{bS,sS}};
}
...
async function runAnalysis() {
    ...
    try{
        ...
        const ai = await askAI({price:price.toFixed(2),bS:sig.scores.bS,sS:sig.scores.sS,zone:sig.zone?.src||'None'});
        
        let dir, conf, entry, sl, tp1, tp2, tp3, reason, src;
        if(ai&&ai.signal!=='NEUTRAL'){ dir=ai.signal; conf=ai.confidence; entry=ai.entryPrice||sig.zone?.p||price; sl=ai.stopLoss||sig.sl; tp1=ai.takeProfit1; tp2=ai.takeProfit2; tp3=ai.takeProfit3; reason=ai.reasoning; src='AI'; }
        else { dir=sig.dir; conf=sig.conf; entry=sig.zone?.p||price; sl=sig.sl; src=sig.zone?.src||'N/A'; const risk=Math.abs(entry-sl); tp1=dir==='BUY'?entry+risk*2:entry-risk*2; tp2=dir==='BUY'?entry+risk*3.5:entry-risk*3.5; tp3=dir==='BUY'?entry+risk*5:entry-risk*5; reason=sig.reason; }
        
        const st = dir==='BUY'?'LONG':(dir==='SELL'?'SHORT':'NEUTRAL');
        const prec = pair.includes('XAU')?2:(['EUR/USD','GBP/USD'].includes(pair)?5:2);
...
function updateLimitUI() {
    ...
    if(limitOrder){
        const prec = pair.includes('XAU')?2:5;
        t.innerHTML = `⏳ ${limitOrder.signalType} LIMIT @ $${limitOrder.idealEntry.toFixed(prec)}`;
...
function handleLimit() {
    ...
    showNotif(`📝 Limit @ $${o.idealEntry.toFixed(pair.includes('XAU')?2:5)}`,'info');
}
